...
{
    public class TrackedAppointmentsContainerRepository : AbstractBaseTrackedContainerRepository<Appointment>
    {
        private readonly DiaryDBContext _dbContext;

        public TrackedAppointmentsContainerRepository(DiaryDBContext dbContext) : base(dbContext)
        {
            _dbContext = dbContext ?? throw new ArgumentNullException(nameof(dbContext));
        }

        public IList<ToDo<ScheduleSettings>> GetTrackedList(int userId, DateTime startDate,
            DateTime finishDate)
        {
            var appointments = (from u in _dbContext.Users
                join c in _dbContext.Categories on u.Id equals c.UserId
                join a in _dbContext.Appointments on c.Id equals a.CategoryId
                join s in _dbContext.Schedules on a.ScheduleId equals s.Id
                    into result
