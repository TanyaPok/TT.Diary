...
namespace TT.Diary.BusinessLogic.Dictionaries.Categories.Commands
{
    public class RemoveHandler : AsyncRequestHandler<RemoveCommand>
    {
        ...
        {
            var category = _context.GetRecursively<Category>(request.Id, c => c.Subcategories);
        ...
        private void RemoveSubcategory(Category category)
        {
            ...
            {
                RemoveSubcategory(category.Subcategories[i]);
                _context.Remove(category.Subcategories[i]);
            }
        }
    }
}