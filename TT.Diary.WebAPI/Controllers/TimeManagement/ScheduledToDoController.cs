...
using TT.Diary.BusinessLogic.TimeManagement.ScheduledToDo.Commands;

namespace TT.Diary.WebAPI.Controllers.TimeManagement
{
    public class ScheduledToDoController : ApiControllerBase
    {
        public ScheduledToDoController(ILogger<ScheduledToDoController> logger, IMediator mediator) : base(logger, mediator)
        {
        }
        ...
        [HttpDelete("withowner/{id}")]
        [ProducesResponseType(200)]
        [ProducesResponseType(400)]
        public async Task<ActionResult<int>> RemoveAllAsync(int id)
        {
            try
            {
                return Ok(await CommandAsync(new RemoveCommand() { OwnerId = id, WithOwner = true }));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<int>> RemoveAsync(int id)
        {
            try
            {
                return Ok(await CommandAsync(new RemoveCommand() { OwnerId = id, WithOwner = false }));
            }
            catch (Exception ex)
            {
                return BadRequest(string.Format(CLIENT_ERROR_MESSAGE, ErrorMessages.RemoveScheduleSettings.GetDescription(), ex.Message));
